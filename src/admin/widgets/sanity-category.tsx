...
import { useSanityDocument, useTriggerSanityCategorySync } from "../hooks/sanity"

const CategoryWidget = ({ data }: DetailWidgetProps<HttpTypes.AdminProductCategory>) => {
  const { mutateAsync, isPending } = useTriggerSanityCategorySync(data.id)
  ...
  const handleSync = async () => {
    ...
    try {
      await mutateAsync()
      toast.success("Category synced to Sanity", { id: tid })
    } catch (e) {
    ...
    <Container className="p-0 overflow-hidden mt-4">
      <div className="flex items-center justify-between px-6 py-4 border-b">
        <div className="flex items-center gap-x-2">
          <h2 className="txt-compact-xlarge-plus text-ui-fg-base">Sanity Status</h2>
          {isLoading ? (
            <span className="animate-pulse txt-compact-small text-ui-fg-muted">Checking...</span>
      ...
      <div className="px-6 py-4 flex gap-x-3">
        ...
        {studio_url && (
          <Button variant="transparent" size="small" asChild>
            <a href={studio_url} target="_blank" rel="noreferrer" className="flex items-center gap-x-2">
              <ArrowUpRightOnBox /> 
              Open in Studio
            </a>
          </Button>
