...
import { useSanityDocument, useTriggerSanityTypeSync } from "../hooks/sanity"

const TypeWidget = ({
  data,
}: DetailWidgetProps<HttpTypes.AdminProductType>) => {
  const { mutateAsync, isPending } = useTriggerSanityTypeSync(data.id)
  const { sanity_document, studio_url, isLoading, refetch } =
    useSanityDocument(data.id)

  const [showCode, setShowCode] = useState(false)

  const handleSync = async () => {
    const tid = toast.loading("Syncing type...")

    try {
      await mutateAsync()
      await refetch()
      toast.success("Type synced to Sanity", { id: tid })
    } catch {
